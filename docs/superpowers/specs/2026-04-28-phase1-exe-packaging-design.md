# Andent Web — MVP Expansion Design
...
Andent Web currently runs from source via `uvicorn app.main:app`. There is no packaging, distribution, or CI/CD infrastructure. The goal is to deliver a standalone Windows executable — double-clickable, system-tray resident, with local data storage — matching the YF_ERP deployment pattern.
...
**Data directory:** `%APPDATA%\Andent Web\` (created on first launch)

```
%APPDATA%/Andent Web/
├── config.env          # Runtime configuration (port, LAN settings, PreForm path)
├── data/
│   ├── andent.sqlite3  # Main SQLite database
│   ├── uploads/        # STL files
...
| Menu Item | Action |
|-----------|--------|
| Open Andent Web | Opens `http://localhost:8090` in browser |
...
```
dist/
└── Andent_Web_v{version}.exe   # Single self-contained executable
```
...
1. Create `%APPDATA%\Andent Web\` if not exists
2. Create subdirectories: `data/`, `data/uploads/`, `data/screenshots/`, `data/backups/`, `data/logs/`
3. Create `config.env` with defaults:
   ```
   PORT=8090
   ALLOW_LAN_ACCESS=true
   PREFORM_PATH=%APPDATA%\Andent Web\PreFormServer
   ```
4. Initialize SQLite at `data/endent.sqlite3`
...
- First launch of EXE detects no `%APPDATA%\Andent Web\` exists
- Shows one-time dialog: *"Where should data be stored?"*
  - **New location** (default): `%APPDATA%\Andent Web\`
  - **Keep existing**: `d:\Marcus\Desktop\Andent_Webapp\data\`
...
| `%APPDATA%\Andent Web\` read-only | Show error dialog, offer to pick alternate location |
...
- `%APPDATA%\Andent Web\` directory created with correct structure
...
Andent Web needs a way for users to report bugs and suggest features. The goal is a simple in-app feedback form that submits directly to GitHub issues — no admin dashboard, no local tracking beyond what's needed to show confirmation to the user.
...
4. Backend calls GitHub API → creates issue in Andent Web repo
...
- Issue body includes:
  ...
  - Description
  - Attachment filenames (files stored in `%APPDATA%\Andent Web\data\feedback_attachments\`)
