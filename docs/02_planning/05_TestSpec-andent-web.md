...
- Source requirements: `docs/01_requirements/prd-andent-web.md`
...
| Review boundaries | Only low-confidence model type or ambiguous/missing case ID | `docs/01_requirements/prd-andent-web.md` | Guard fixture results and manual validation notes |
...
| 11. Evidence archive | Save logs and outputs under `docs/02_planning/98_VerificationArtifacts/pre_release_YYYYMMDD/`. | Evidence bundle contains all required files listed below. |
...
```text
docs/02_planning/98_VerificationArtifacts/pre_release_YYYYMMDD/
```
...
- Use existing repo fixtures under `docs/04_customer-facing/` and `tests/release_gate/fixtures/` as controlled guard fixtures when the external dataset does not cover a specific negative path.
