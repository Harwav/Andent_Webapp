...
| Unit: PreForm device payload normalization | Virtual mode accepts PreForm's wrapped/string device payload shape and selects the manifest/preferred virtual printer. |
| Unit: PreForm handoff | `send_to_printer` is called only in `virtual` or `real` modes. |
| Integration: happy path | Upload/select/send creates a `print_jobs` row with non-empty `scene_id`, `form_file_path`, `print_job_id`, and `screenshot_url`. |
| Release gate: live PreForm | Browser flow proves a virtual PreForm print job was accepted by the real endpoint. |
| Release gate: operator walkthrough | Headed Playwright uses `D:\Marcus\Desktop\BM\20260409_Andent_Matt\Test Data`, confirms History/Print Queue visibility, and clicks the screenshot/preview zoom modal. |
...
ANDENT_WEB_PRINT_DISPATCH_MODE=virtual npx playwright test --config playwright.config.ts --project=chromium --reporter=line --workers=1
8 passed
```
...
- Virtual mode uses `PreFormClient.list_devices()` before dispatch and normalizes PreForm's `{"count": ..., "devices": [...]}` payload, including JSON-string payloads.
- Virtual and real modes use the existing `PreFormClient.send_to_printer()` method for the endpoint call.
- `tests/test_preform_handoff.py` covers virtual dispatch, wrapped/string device payloads, physical-only refusal, invalid mode rejection, and virtual/debug generated preview access.
- Virtual/debug Print Queue jobs expose a clickable generated PNG preview when Formlabs Web screenshots are unavailable, so the operator walkthrough still proves zoom-modal behavior.
